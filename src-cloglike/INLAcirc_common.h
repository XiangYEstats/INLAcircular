...
{
    return 0.5 * (1.0 + erf(x * INLACIRC_INV_SQRT_2));
}
...
static inline double INLAcirc_chebyshev_eval(const double *coefficients,
                                              int order,
                                              double x)
{
    double d = 0.0;
...
static inline double INLAcirc_bessel_i0_scaled(double x)
{
    ...
    if (y <= 3.0) {
        return exp(-y) *
               (2.75 + INLAcirc_chebyshev_eval(INLAcirc_bi0_data, 11,
                                                y * y / 4.5 - 1.0));
    }
    if (y <= 8.0) {
        return (0.375 + INLAcirc_chebyshev_eval(
                            INLAcirc_ai0_data, 20,
                            (48.0 / y - 11.0) / 5.0)) /
               sqrt(y);
    }
    return (0.375 + INLAcirc_chebyshev_eval(INLAcirc_ai02_data, 21,
                                             16.0 / y - 1.0)) /
           sqrt(y);
}
...
static inline double INLAcirc_bessel_i1_scaled(double x)
{
    ...
    if (y <= 3.0) {
        return x * exp(-y) *
               (0.875 + INLAcirc_chebyshev_eval(INLAcirc_bi1_data, 10,
                                                 y * y / 4.5 - 1.0));
    }
    if (y <= 8.0) {
        return sign *
               (0.375 + INLAcirc_chebyshev_eval(
                            INLAcirc_ai1_data, 20,
                            (48.0 / y - 11.0) / 5.0)) /
               sqrt(y);
    }
    return sign *
           (0.375 + INLAcirc_chebyshev_eval(INLAcirc_ai12_data, 21,
                                             16.0 / y - 1.0)) /
           sqrt(y);
}
 ...
 */
static inline double INLAcirc_log_bessel_i0_scaled_from_log_kappa(
    double log_kappa)
{
    if (log_kappa > 11.512925) {
        return -0.5 * (INLACIRC_LOG_2PI + log_kappa) +
               0.125 * exp(-log_kappa);
    }
